...
Table [[tab:linearRegTab10]] shows regression results of a qudratic fit to explain
...
[[fig:linearRegFig13]], we observe that removign the high leverage observation has a
...
#+CAPTION: Scatterplots of the observations from the =Credit= data set.  Left: A plot of =age= versus =limit=.  These two variables not collinear.  Right: A plot of =rating= versus =limit=.  There is high collinearity.
#+RESULTS: fig3_14plot
