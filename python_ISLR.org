...
interval.  The right panel shows a model fitted using logistic regresison, which
...
#+CAPTION: Left: Two one-dimensional normal density functions are shown.  The dashed vertical line represents the Bayes decision boundary.  Right: 50 observations were drawn from each of the two classes, and are shown as histograms.  The Bayes decision boundary is again shown as a dashed vertical line.  The solid vertical line represents the LDA decision boundary estimated from the training data.
#+RESULTS: fig4_4plot
