...
recover_2s10s_all = recover_2s10s_pc1 + \
    recover_2s10s_pc2 + np.array(mean_2s10s)[np.newaxis, :]
...
treasury_diff.plot(x='10 YR', y='2 YR', kind='scatter',
                   c='r', ax=ax, alpha=0.7)
ax.plot(recover_2s10s_pc1[:, 1], recover_2s10s_pc1[:, 0], c='g', label='pc1',
        linewidth=1)
ax.plot(recover_2s10s_pc2[:, 1], recover_2s10s_pc2[:, 0], c='b',
        linestyle=':', label='pc2')
# ax.scatter(recover_2s10s_all[:, 1], recover_2s10s_all[:, 0], c='k', marker='x',
#            alpha=0.7)
ax.legend()
