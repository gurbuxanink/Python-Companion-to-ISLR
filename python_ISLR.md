...
1.  [Introduction](#org8f63d04)
2.  [Statistical Learning](#orgb5180c5)
    1.  [What is Statistical Learning?](#org5e76816)
    2.  [Assessing Model Accuracy](#org6ea50d2)
...
Figure [2](#org4f9813f) shows graphs of Wage versus three variables. 
...
Figure [4](#org631149a) shows boxplots of previous days' percentage changes in S&P
...
Figure [6](#org945e510) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [8](#orga4dcdae) is a plot of `Income` versus `Years of Education` from the
...
Figure [10](#org908bed9) is a plot of `Income` versus `Years of Education` and
...
Figure [12](#orgea1dfa7) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [10](#org908bed9).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [14](#orgb24916d) provides an illustration of the trade-off between
...
Figure [16](#org8ee56a6) provides a simple illustration of the clustering problem.
...
Figure [20](#orgc06d664) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [20](#orgc06d664), we see considerable
...
Figure [22](#org748b018) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [20](#orgc06d664) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [24](#org8221890) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [20](#orgc06d664), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [26](#orgbd4bd0d) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [20](#orgc06d664) - [24](#org8221890).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")

Figure [28](#org43b37f5) provides an example using a simulated data set in
...
Figure [30](#org235d9e0) displays the KNN decision boundary, using \(K=10\), when
applied to the simulated data set from figure [28](#org43b37f5).  Even though
...
![img](figures/fig2_15.png "The firm line indicates the KNN decision boundary on the data from figure [28](#org43b37f5), using \(K = 10\). The Bayes decision boundary is shown as a dashed line.  The KNN and Bayes decision boundaries are very similar.")

![img](figures/fig2_16.png "A comparison of the KNN decision boundaries (solid curves) obtained using \(K=1\) and \(K=100\) on the data from figure [28](#org43b37f5).  With \(K=1\), the decision boundary is overly flexible, while with \(K=100\) it is not sufficiently flexible.  The Bayes decision boundary is shown as dashed line.")
