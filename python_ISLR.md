...
1.  [Introduction](#org1450662)
2.  [Statistical Learning](#org15bd9a2)
    1.  [What is Statistical Learning?](#org7e31388)
    2.  [Assessing Model Accuracy](#org3b49d96)
    3.  [Lab: Introduction to Python](#org8c801cf)
        1.  [Basic Commands](#org0f6aa77)
        2.  [Graphics](#org4e20c5c)
        3.  [Indexing Data](#org4496628)
        4.  [Loading Data](#org05d9e13)
        5.  [Additional Graphical and Numerical Summaries](#org2bd1c85)
3.  [Linear Regression](#org3b6713d)
    1.  [Simple Linear Regression](#orgf7dbf9f)
    2.  [Multiple Linear Regression](#org0b375c6)
    3.  [Other Considerations in the Regression Model](#org18fdab1)
    4.  [The Marketing Plan](#org585b081)
    5.  [Comparison of Linear Regression with K-Nearest Neighbors](#org6446746)
    6.  [Lab: Linear Regression](#org5cd8931)
        1.  [Libraries](#orgac29785)
        2.  [Simple Linear Regression](#org7c3413c)
        3.  [Multiple Linear Regression](#org34fdc89)
        4.  [Interaction Terms](#orgbb16858)
        5.  [Non-linear Transformations of the Predictors](#org8b1a477)
        6.  [Qualitative Predictors](#org5f9ebe8)
        7.  [Calling `R` from `Python`](#orgd0647e0)
4.  [Classification](#org7fd1830)
    1.  [An Overview of Classification](#orgd2e754a)
    2.  [Why Not Linear Regression?](#org21edc2a)
    3.  [Logistic Regression](#org6ad4caa)
    4.  [Linear Discriminant Analysis](#org03bcf91)
    5.  [A Comparison of Classification Methods](#org5302368)
    6.  [Lab: Logistic Regression, LDA, QDA, and KNN](#org42f6b93)
        1.  [The Stock Market Data](#org53364b3)
        2.  [Logistc Regression](#org48d6fb5)
        3.  [Linear Discriminant Analysis](#org782783b)
        4.  [Quadratic Discriminant Analysis](#orgb888c6e)
        5.  [K-Nearest Neightbors](#org235c0e3)
        6.  [An Application to Caravan Insurance Data](#org731d0e0)
5.  [Resampling Methods](#org667599e)
    1.  [Cross-Validation](#orga8a0434)
...
Figure [2](#org40d4ab7) shows graphs of Wage versus three variables. 
...
Figure [4](#orgb0e21ac) shows boxplots of previous days' percentage changes in S&P
...
Figure [7](#org5059f00) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [9](#org0b11d9d) is a plot of `Income` versus `Years of Education` from the
...
Figure [11](#org5bb31a2) is a plot of `Income` versus `Years of Education` and
...
Figure [13](#orgbfa621b) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [11](#org5bb31a2).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [15](#orgad454c8) provides an illustration of the trade-off between
...
Figure [17](#org78ab4ac) provides a simple illustration of the clustering problem.
...
Figure [21](#orge6772a7) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [21](#orge6772a7), we see considerable
...
Figure [23](#org655f5c8) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [21](#orge6772a7) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [25](#orgc9bfb52) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [21](#orge6772a7), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [27](#orgcd848c3) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [21](#orge6772a7) - [25](#orgc9bfb52).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")

Figure [29](#orge5ecb10) provides an example using a simulated data set in
...
Figure [31](#orgdef2aea) displays the KNN decision boundary, using \(K=10\), when
applied to the simulated data set from figure [29](#orge5ecb10).  Even though
...
![img](figures/fig2_15.png "The firm line indicates the KNN decision boundary on the data from figure [29](#orge5ecb10), using \(K = 10\). The Bayes decision boundary is shown as a dashed line.  The KNN and Bayes decision boundaries are very similar.")

![img](figures/fig2_16.png "A comparison of the KNN decision boundaries (solid curves) obtained using \(K=1\) and \(K=100\) on the data from figure [29](#orge5ecb10).  With \(K=1\), the decision boundary is overly flexible, while with \(K=100\) it is not sufficiently flexible.  The Bayes decision boundary is shown as dashed line.")

In figure [34](#orgc2e2d6f) we have plotted the KNN test and training errors as
...
![img](figures/fig2_17.png "The KNN training error rate (blue, 200 observations) and test error rate (orange, 5,000 observations) on the data from figure [29](#orge5ecb10) as the level of flexibility (assessed using \(\frac{1}{K}\)) increases, or equivalently as the number of neighbors \(K\) decreases.  The black dashed line indicates the Bayes error rate.")
...
Figure [48](#org33498ac) displays the simple linear regression fit to the
...
In figure [50](#org22fd688), we have computed RSS for a number of values of
...
The left-hand panel of figure [52](#org41e679e) displays *population regression
...
right-hand panel of figure [52](#org41e679e) we have generated five different
...
For `Advertising` data, table [1](#org12a4c39) provides details of the least squares model for the
regression of number of units sold on TV advertising budget. 

<table id="org12a4c39" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Next, in table [2](#org7358b2e), we report more information about the least squares model.  

<table id="org7358b2e" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [3](#orgd93fba7)  shows results of two simple linear
...
<table id="orgd93fba7" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [57](#org9fbca73) illustrates an example of the least squares fit to a
...
Table [4](#org1aa4b06) displays multiple regression coefficient estimates when
TV, radio, and newspaper advertising budgets are used to predict product sales
using `Advertising` data.

<table id="org1aa4b06" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [5](#orgeabc680) shows the correlation matrix for the three predictor
variables and response variable in table [4](#org1aa4b06). 

<table id="orgeabc680" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="org88aab51" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 6:</span> More information about the least squares model for the regression of number of units sold on TV, newspaper, and radio advertising budgets in the `Advertising` data.  Other information about this model was displayed in table [4](#org1aa4b06).</caption>
...
Figure [61](#orgd74d02f) displays a three-dimensional plot of `TV` and `radio`
versus `sales`.  
...
`Credit` data set displayed in figure [63](#org2d9c5aa) records `balance`
...
Table [7](#orgfb0b380) displays the coefficient estimates and other information
associated with the model where `gender` is the only explanatory variable.

<table id="orgfb0b380" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
From table [8](#org5a009ed) we see that the estimated `balance` for the
...
<table id="org5a009ed" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [9](#orga8e485d) shows results of regressing `sales` and `TV` and `radio`
when an interaction term is included.  Coefficient of interaction term
`TV:radio` is highly significant.

In figure [68](#org17fd390), the left panel shows least squares lines when
...
<table id="orga8e485d" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [70](#orgf6e597a) shows a scatter plot of `mpg` (gas mileage in miles per
...
Table [10](#org556cc97) shows regression results of a quadratic fit to explain
`mpg` as a function of `horsepower` and \(\mathttt{horsepower^2}\).  

<table id="org556cc97" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
The left panel of figure [73](#org0b875b1) displays a residual plot from the
...
of figure[73](#org0b875b1) displays the residual plot results from the model
...
Figure [75](#orgd2265d4) provides an illustration of correlations among
...
In the left-hand panel of figure [77](#org26d41a6), the magnitude of the
...
[79](#orge1a9ed9) illustrates a typical outlier.  The red solid line is the
...
[79](#orge1a9ed9), the outlier is clearly visible.  In practice, to decide if
...
Observation 41 in the left-hand panel in figure [81](#orgf3c4e00) has
...
[81](#orgf3c4e00) are the same as the data displayed in figure
[79](#orge1a9ed9), except for the addition of a single high leverage
...
removed. Comparing the left-hand panels of figures [79](#orge1a9ed9) and
[81](#orgf3c4e00), we observe that removing the high leverage observation has a
much more substantial impact on least squares line than removing the outlier.
The center panel of figure [81](#orgf3c4e00), for a data set with two
...
point. The right-panel of figure [81](#orgf3c4e00) provides a plot of
...
Figure [83](#orgd87ed23) illustrates the concept of collinearity.
...
Figure [86](#orgdb3f843) illustrates some of the difficulties that can result
...
In contrast, the right hand panel of figure [86](#orgdb3f843) displays contour
...
Table [11](#orge756a96) compares the coefficient estimates obtained from two
...
<table id="orge756a96" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [89](#org6150fc5) illustrates two KNN fits on a data set with \(p = 2\)
...
Figure [91](#orgdf2443d) provides an example of KNN regression with data
...
Figure [93](#orgfdf76d5) represents the linear regression fit to the same
data.  It is almost perfect.  The right hand panel of figure [93](#orgfdf76d5)
...
![img](figures/fig3_18.png "The same data set shown in figure [91](#orgdf2443d) is investigated further.  Left: The blue dashed line is the least squares fit to the data.  Since \(f(X)\) is in fact linear (displayed in black line), the least squares regression line provides a very good estimate of \(f(X)\).  Right: The dashed horizontal line represents the least squares test set MSE, while the green line corresponds to the MSE for KNN as a function of \(\frac{1}{K}\).  Linear regression achieves a lower test MSE than does KNN regression, since \(f(X)\) is in fact linear.")

Figure [95](#org9118754) examines the relative performances of least squares
...
Figure [97](#org82c5134) considers the same strongly non-linear situation as in the lower
panel of figure [95](#org9118754), except that we have added additional *noise*
...
In figure [110](#org5ac0e43), we have plotted annual `income` and monthly
...
Using `Default` data set, in figure [112](#orgbb25a0b) we show probability of default as a function of
...
Table [12](#orgd7edd25) shows the coefficient estimates and related
...
<table id="orgd7edd25" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [13](#org544fdc7) shows the results of logistic model where `default`
is a function of the qualitative variable `student`.  

Table [14](#orgb362703) shows the coefficient estimates for a logistic
regression model that uses `balance`, `income` (in thousands of dollars), and
`student` status to predict probability of `default`.

<table id="org544fdc7" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="orgb362703" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
The left hand panel of figure [117](#org4bc6ab5) shows average default rates
...
[14](#orgb362703).  But the horizontal lines near the base of the plot, which show the default rates
...
In the left panel of figure [119](#org5650ae1), two normal density functions
...
figure [121](#org31fdac2).  In the upper panel, the height of the surface at
...
right-hand panel of figure [121](#org31fdac2).  In this situation, the base
...
Figure [123](#org0c7d159) shows an example of three equally sized Gaussian
...
[15](#org8156982), is a convenient way to display prediction of default in
comparison to true default.  Table [16](#org84edd05) shows the error rates
that result when we label any customer with a posterior probability of default
above 20% to the *default* class.

<table id="org8156982" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="org84edd05" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [126](#orgef486bf) illustrates the trade-off that results from
...
Figure [128](#org736de21) displays the ROC curve for the LDA classifier on
...
Table [17](#orgd723091) shows the possible results when applying a
classifier (or diagnostic test) to a population.  

<table id="orgd723091" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [18](#orga6ac7d5) lists many of the popular performance measures that
are used in this context.

<table id="orga6ac7d5" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 18:</span> Important measures for classification and diagnostic testing, derived from quantities in table [17](#orgd723091).</caption>
...
Figure [132](#org9e38ee2) illustrates the performances of LDA and QDA in two
...
Figure [134](#org4fe67b6) illustrates the performances of the four
...
Figure [143](#org75296ce) displays the *validation set approach*, a simple
...
In figure [145](#orge6a670e), the left-hand panel shows validation sample MSE as a
...
Figure [147](#orga3ed0be) displas the Leave One Out Cross Validation (LOOCV) approach.
...
The left-hand panel of figure [149](#orgfa778d1) shows test set MSE as a function of
...
The right-hand panel of figure [149](#orgfa778d1) shows nine different 10-fold
...
Figure [151](#orgb60ccb6) illustrates the *k*-fold CV approach.
...
In figure [153](#org654208e), we plot the cross-validation estimates and true
test error rates that result from fitting least squares polynomials to the
simulated data sets illustrated in figures [21](#orge6772a7),
[23](#org655f5c8), and [25](#orgc9bfb52) of chapter [2](#org68d9728).  In all
three plots, the two cross validation errors are very similar.  

![img](figures/fig5_6.png "True and estimated test MSE for the simulated data sets in Figures [21](#orge6772a7) (left), [23](#org655f5c8) (center), and [25](#orgc9bfb52) (right).  The true test MSE is shown in blue, the LOOCV estimate is shown in black dashed line, and the 10-fold CV estimate is shown in red dotted line.")
