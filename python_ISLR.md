...
1.  [Introduction](#orgb14abd2)
2.  [Statistical Learning](#orgcfdb5cd)
    1.  [What is Statistical Learning?](#org60e3a0e)
    2.  [Assessing Model Accuracy](#org2dc189e)
    3.  [Lab: Introduction to Python](#org08659ff)
        1.  [Basic Commands](#org7a5a791)
        2.  [Graphics](#org8080132)
        3.  [Indexing Data](#org4c3d8d9)
        4.  [Loading Data](#org371de86)
        5.  [Additional Graphical and Numerical Summaries](#org4f8162c)
3.  [Linear Regression](#org8c16b80)
    1.  [Simple Linear Regression](#org9022e24)
    2.  [Multiple Linear Regression](#org652ee41)
...
Figure [2](#orgbbde094) shows graphs of Wage versus three variables. 
...
Figure [4](#org9e6fdf1) shows boxplots of previous days' percentage changes in S&P
...
Figure [6](#org05f605b) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [8](#orgf1957cb) is a plot of `Income` versus `Years of Education` from the
...
Figure [10](#org07cb349) is a plot of `Income` versus `Years of Education` and
...
Figure [12](#orgdf4f4cb) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [10](#org07cb349).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [14](#org0fb3c1e) provides an illustration of the trade-off between
...
Figure [16](#org58cefcb) provides a simple illustration of the clustering problem.
...
Figure [20](#orgb55ec80) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [20](#orgb55ec80), we see considerable
...
Figure [22](#org4b564c8) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [20](#orgb55ec80) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [24](#org1b2b98f) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [20](#orgb55ec80), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [26](#org21f2dba) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [20](#orgb55ec80) - [24](#org1b2b98f).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")

Figure [28](#org3be70b4) provides an example using a simulated data set in
...
Figure [30](#orgae8667d) displays the KNN decision boundary, using \(K=10\), when
applied to the simulated data set from figure [28](#org3be70b4).  Even though
...
![img](figures/fig2_15.png "The firm line indicates the KNN decision boundary on the data from figure [28](#org3be70b4), using \(K = 10\). The Bayes decision boundary is shown as a dashed line.  The KNN and Bayes decision boundaries are very similar.")

![img](figures/fig2_16.png "A comparison of the KNN decision boundaries (solid curves) obtained using \(K=1\) and \(K=100\) on the data from figure [28](#org3be70b4).  With \(K=1\), the decision boundary is overly flexible, while with \(K=100\) it is not sufficiently flexible.  The Bayes decision boundary is shown as dashed line.")

In figure [33](#org5d40459) we have plotted the KNN test and training errors as
...
![img](figures/fig2_17.png "The KNN training error rate (blue, 200 observations) and test error rate (orange, 5,000 observations) on the data from figure [28](#org3be70b4) as the level of flexibility (assessed using \(\frac{1}{K}\)) increases, or equivalently as the number of neighbors \(K\) decreases.  The black dashed line indicates the Bayes error rate.")
...
Figure [47](#org17308bc) displays the simple linear regression fit to the
...
In figure [49](#org7e226e0), we have computed RSS for a number of values of
...
The left-hand panel of figure [51](#orge6b93c3) displays *population regression
...
right-hand panel of figure [51](#orge6b93c3) we have generated five different
...
For `Advertising` data, table [1](#org0c1ad6b) provides details of the least squares model for the
regression of number of units sold on TV advertising budget. 

<table id="org0c1ad6b" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 1:</span> For `Advertising` data, the coefficients of the least squares model</caption>
...
Next, in table [2](#org4b25b6a), we report more information about the least squares model.  

<table id="org4b25b6a" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
