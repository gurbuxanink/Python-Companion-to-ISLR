...
1.  [Introduction](#org95ffd87)
2.  [Statistical Learning](#org4731909)
    1.  [What is Statistical Learning?](#org9bb14b1)
    2.  [Assessing Model Accuracy](#org60e4d6f)
    3.  [Lab: Introduction to Python](#org64fffd0)
        1.  [Basic Commands](#orgea20ee9)
        2.  [Graphics](#org2626a50)
        3.  [Indexing Data](#org524faa8)
        4.  [Loading Data](#orgdcb2770)
        5.  [Additional Graphical and Numerical Summaries](#org1a0ee5a)
3.  [Linear Regression](#org9cb77ee)
    1.  [Simple Linear Regression](#org3f50281)
    2.  [Multiple Linear Regression](#org4a2e4a4)
...
Figure [2](#org8737735) shows graphs of Wage versus three variables. 
...
Figure [4](#orge379851) shows boxplots of previous days' percentage changes in S&P
...
Figure [6](#org36001bf) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [8](#org1e2ccde) is a plot of `Income` versus `Years of Education` from the
...
Figure [10](#orgd19efff) is a plot of `Income` versus `Years of Education` and
...
Figure [12](#org3508201) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [10](#orgd19efff).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [14](#org1af5863) provides an illustration of the trade-off between
...
Figure [16](#org74a3f58) provides a simple illustration of the clustering problem.
...
Figure [20](#orgeaeff93) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [20](#orgeaeff93), we see considerable
...
Figure [22](#org91dc0c4) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [20](#orgeaeff93) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [24](#org66afee1) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [20](#orgeaeff93), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [26](#org1b34cfd) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [20](#orgeaeff93) - [24](#org66afee1).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")

Figure [28](#org26f8d8a) provides an example using a simulated data set in
...
Figure [30](#orgfebf97e) displays the KNN decision boundary, using \(K=10\), when
applied to the simulated data set from figure [28](#org26f8d8a).  Even though
...
![img](figures/fig2_15.png "The firm line indicates the KNN decision boundary on the data from figure [28](#org26f8d8a), using \(K = 10\). The Bayes decision boundary is shown as a dashed line.  The KNN and Bayes decision boundaries are very similar.")

![img](figures/fig2_16.png "A comparison of the KNN decision boundaries (solid curves) obtained using \(K=1\) and \(K=100\) on the data from figure [28](#org26f8d8a).  With \(K=1\), the decision boundary is overly flexible, while with \(K=100\) it is not sufficiently flexible.  The Bayes decision boundary is shown as dashed line.")

In figure [33](#org9c185d2) we have plotted the KNN test and training errors as
...
![img](figures/fig2_17.png "The KNN training error rate (blue, 200 observations) and test error rate (orange, 5,000 observations) on the data from figure [28](#org26f8d8a) as the level of flexibility (assessed using \(\frac{1}{K}\)) increases, or equivalently as the number of neighbors \(K\) decreases.  The black dashed line indicates the Bayes error rate.")
...
Figure [47](#org3fb43b8) displays the simple linear regression fit to the
...
In figure [49](#org358468b), we have computed RSS for a number of values of
...
The left-hand panel of figure [51](#orge895b8f) displays *population regression
...
right-hand panel of figure [51](#orge895b8f) we have generated five different
...
For `Advertising` data, table [1](#org6102287) provides details of the least squares model for the
regression of number of units sold on TV advertising budget. 

<table id="org6102287" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Next, in table [2](#org177ab47), we report more information about the least squares model.  

<table id="org177ab47" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [3](#org3f20188) and the next table show results of two simple linear
...
<table id="org3f20188" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [56](#org5778060) illustrates an example of the least squares fit to a
...
Table [5](#org9e55ebc) displays multiple regression coefficient estimates when
TV, radio, and newspaper advertising budgets are used to predict product sales
using `Advertising` data.

<table id="org9e55ebc" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [6](#orgc4fbd53) shows the correlation matrix for the three predictor
variables and response variable in table [5](#org9e55ebc). 

<table id="orgc4fbd53" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="org093cfab" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 6:</span> More information about the least squares model for the regression of number of units sold on TV, newspaper, and radio advertising budgets in the `Advertising` data.  Other information about this model was displayed in table [5](#org9e55ebc).</caption>
...
Figure [60](#org00f5ca1) displays a three-dimensional plot of `TV` and `radio`
versus `sales`.  
