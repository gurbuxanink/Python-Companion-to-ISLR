...
1.  [Introduction](#orgfb21996)
2.  [Statistical Learning](#org44433d3)
    1.  [What is Statistical Learning?](#orgc2eb084)
    2.  [Assessing Model Accuracy](#org3fccdbe)
    3.  [Lab: Introduction to Python](#org3dc0332)
        1.  [Basic Commands](#org21ce202)
        2.  [Graphics](#org5eb2be0)
        3.  [Indexing Data](#org3ea2405)
        4.  [Loading Data](#orgd1c434b)
        5.  [Additional Graphical and Numerical Summaries](#org164dd0e)
3.  [Linear Regression](#org537eefc)
    1.  [Simple Linear Regression](#org70f34fc)
    2.  [Multiple Linear Regression](#org7b438e3)
    3.  [Other Considerations in the Regression Model](#org9e82bd8)
    4.  [The Marketing Plan](#orgaba23b3)
    5.  [Comparison of Linear Regression with K-Nearest Neighbors](#orgaaeb28c)
    6.  [Lab: Linear Regression](#org7a14990)
        1.  [Libraries](#org55e9650)
        2.  [Simple Linear Regression](#org0bce938)
        3.  [Multiple Linear Regression](#org8d3c265)
        4.  [Interaction Terms](#orgb0f3613)
        5.  [Non-linear Transformations of the Predictors](#orgc7fbae1)
        6.  [Qualitative Predictors](#orge1de91d)
        7.  [Calling `R` from `Python`](#org9abfcdd)
4.  [Classification](#orgd9b92e1)
    1.  [An Overview of Classification](#org402b811)
    2.  [Why Not Linear Regression?](#org1f9aa3f)
    3.  [Logistic Regression](#orgf450bef)
    4.  [Linear Discriminant Analysis](#orgd4ea4fa)
    5.  [A Comparison of Classification Methods](#org8df0d0b)
    6.  [Lab: Logistic Regression, LDA, QDA, and KNN](#org1ac23ae)
        1.  [The Stock Market Data](#org3fd3f21)
        2.  [Logistc Regression](#orga563f8c)
        3.  [Linear Discriminant Analysis](#org56df920)
        4.  [Quadratic Discriminant Analysis](#orgcc6e052)
        5.  [K-Nearest Neightbors](#org3b93f1d)
        6.  [An Application to Caravan Insurance Data](#org4cdcc59)
5.  [Resampling Methods](#orgacf8f8a)
    1.  [Cross-Validation](#org6e165ae)
...
Figure [2](#orgf248d73) shows graphs of Wage versus three variables. 
...
Figure [4](#org62515e8) shows boxplots of previous days' percentage changes in S&P
...
Figure [6](#org06eaff7) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [8](#org2c2b4f2) is a plot of `Income` versus `Years of Education` from the
...
Figure [10](#orgab77405) is a plot of `Income` versus `Years of Education` and
...
Figure [12](#org036d660) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [10](#orgab77405).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [14](#orgb5b7fdb) provides an illustration of the trade-off between
...
Figure [16](#orgd22f52a) provides a simple illustration of the clustering problem.
...
Figure [20](#org07155df) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [20](#org07155df), we see considerable
...
Figure [22](#orgc97c037) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [20](#org07155df) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [24](#orgb60edc6) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [20](#org07155df), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [26](#org68f3574) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [20](#org07155df) - [24](#orgb60edc6).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")

Figure [28](#org574b500) provides an example using a simulated data set in
...
Figure [30](#org25180f5) displays the KNN decision boundary, using \(K=10\), when
applied to the simulated data set from figure [28](#org574b500).  Even though
...
![img](figures/fig2_15.png "The firm line indicates the KNN decision boundary on the data from figure [28](#org574b500), using \(K = 10\). The Bayes decision boundary is shown as a dashed line.  The KNN and Bayes decision boundaries are very similar.")

![img](figures/fig2_16.png "A comparison of the KNN decision boundaries (solid curves) obtained using \(K=1\) and \(K=100\) on the data from figure [28](#org574b500).  With \(K=1\), the decision boundary is overly flexible, while with \(K=100\) it is not sufficiently flexible.  The Bayes decision boundary is shown as dashed line.")

In figure [33](#org8e6c943) we have plotted the KNN test and training errors as
...
![img](figures/fig2_17.png "The KNN training error rate (blue, 200 observations) and test error rate (orange, 5,000 observations) on the data from figure [28](#org574b500) as the level of flexibility (assessed using \(\frac{1}{K}\)) increases, or equivalently as the number of neighbors \(K\) decreases.  The black dashed line indicates the Bayes error rate.")
...
Figure [47](#orgaa197d2) displays the simple linear regression fit to the
...
In figure [49](#orgb057655), we have computed RSS for a number of values of
...
The left-hand panel of figure [51](#org5f0a67b) displays *population regression
...
right-hand panel of figure [51](#org5f0a67b) we have generated five different
...
For `Advertising` data, table [1](#orgd73d8b5) provides details of the least squares model for the
regression of number of units sold on TV advertising budget. 

<table id="orgd73d8b5" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Next, in table [2](#orgc0578d8), we report more information about the least squares model.  

<table id="orgc0578d8" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [3](#org1ae1580)  shows results of two simple linear
...
<table id="org1ae1580" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [56](#orgb856a0a) illustrates an example of the least squares fit to a
...
Table [4](#org6af4650) displays multiple regression coefficient estimates when
TV, radio, and newspaper advertising budgets are used to predict product sales
using `Advertising` data.

<table id="org6af4650" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [5](#orgf251e08) shows the correlation matrix for the three predictor
variables and response variable in table [4](#org6af4650). 

<table id="orgf251e08" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="orgba31a50" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 6:</span> More information about the least squares model for the regression of number of units sold on TV, newspaper, and radio advertising budgets in the `Advertising` data.  Other information about this model was displayed in table [4](#org6af4650).</caption>
...
Figure [60](#orgfdca09e) displays a three-dimensional plot of `TV` and `radio`
versus `sales`.  
...
`Credit` data set displayed in figure [62](#orgcb2da1d) records `balance`
...
Table [7](#orgeef4eea) displays the coefficient estimates and other information
associated with the model where `gender` is the only explanatory variable.

<table id="orgeef4eea" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
From table [8](#orgc3ae34f) we see that the estimated `balance` for the
...
<table id="orgc3ae34f" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [9](#org41a587c) shows results of regressing `sales` and `TV` and `radio`
when an interaction term is included.  Coefficient of interaction term
`TV:radio` is highly significant.

In figure [67](#org68bfe5e), the left panel shows least squares lines when
...
<table id="org41a587c" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [69](#org790e5ab) shows a scatter plot of `mpg` (gas mileage in miles per
...
Table [10](#org8e0f0fe) shows regression results of a quadratic fit to explain
`mpg` as a function of `horsepower` and \(\mathttt{horsepower^2}\).  

<table id="org8e0f0fe" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
The left panel of figure [72](#org306e408) displays a residual plot from the
...
of figure[72](#org306e408) displays the residual plot results from the model
...
Figure [74](#org7fd5f9a) provides an illustration of correlations among
...
In the left-hand panel of figure [76](#orgfb07309), the magnitude of the
...
[78](#orgcf39e88) illustrates a typical outlier.  The red solid line is the
...
[78](#orgcf39e88), the outlier is clearly visible.  In practice, to decide if
...
Observation 41 in the left-hand panel in figure [80](#orgd694294) has
...
[80](#orgd694294) are the same as the data displayed in figure
[78](#orgcf39e88), except for the addition of a single high leverage
...
removed. Comparing the left-hand panels of figures [78](#orgcf39e88) and
[80](#orgd694294), we observe that removing the high leverage observation has a
much more substantial impact on least squares line than removing the outlier.
The center panel of figure [80](#orgd694294), for a data set with two
...
point. The right-panel of figure [80](#orgd694294) provides a plot of
...
Figure [82](#org4bd748a) illustrates the concept of collinearity.
...
Figure [85](#org8bebeae) illustrates some of the difficulties that can result
...
In contrast, the right hand panel of figure [85](#org8bebeae) displays contour
...
Table [11](#orgc666ebf) compares the coefficient estimates obtained from two
...
<table id="orgc666ebf" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [88](#org103f41e) illustrates two KNN fits on a data set with \(p = 2\)
...
Figure [90](#orge64e563) provides an example of KNN regression with data
...
Figure [92](#orgb417302) represents the linear regression fit to the same
data.  It is almost perfect.  The right hand panel of figure [92](#orgb417302)
...
![img](figures/fig3_18.png "The same data set shown in figure [90](#orge64e563) is investigated further.  Left: The blue dashed line is the least squares fit to the data.  Since \(f(X)\) is in fact linear (displayed in black line), the least squares regression line provides a very good estimate of \(f(X)\).  Right: The dashed horizontal line represents the least squares test set MSE, while the green line corresponds to the MSE for KNN as a function of \(\frac{1}{K}\).  Linear regression achieves a lower test MSE than does KNN regression, since \(f(X)\) is in fact linear.")

Figure [94](#orge840601) examines the relative performances of least squares
...
Figure [96](#org442b19a) considers the same strongly non-linear situation as in the lower
panel of figure [94](#orge840601), except that we have added additional *noise*
...
In figure [109](#org0349362), we have plotted annual `income` and monthly
...
Using `Default` data set, in figure [111](#org0c6b634) we show probability of default as a function of
...
Table [12](#org51011b4) shows the coefficient estimates and related
...
<table id="org51011b4" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [13](#org5e7fecf) shows the results of logistic model where `default`
is a function of the qualitative variable `student`.  

Table [14](#org95c6b37) shows the coefficient estimates for a logistic
regression model that uses `balance`, `income` (in thousands of dollars), and
`student` status to predict probability of `default`.

<table id="org5e7fecf" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="org95c6b37" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
The left hand panel of figure [116](#org64a68cd) shows average default rates
...
[14](#org95c6b37).  But the horizontal lines near the base of the plot, which show the default rates
...
In the left panel of figure [118](#org4e0e1b6), two normal density functions
...
figure [120](#org770fe32).  In the upper panel, the height of the surface at
...
right-hand panel of figure [120](#org770fe32).  In this situation, the base
...
Figure [122](#orgf3c534f) shows an example of three equally sized Gaussian
...
[15](#org521e885), is a convenient way to display prediction of default in
comparison to true default.  Table [16](#orgbbb42bf) shows the error rates
that result when we label any customer with a posterior probability of default
above 20% to the *default* class.

<table id="org521e885" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
<table id="orgbbb42bf" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Figure [125](#orgdc885c2) illustrates the trade-off that results from
...
Figure [127](#orge14b470) displays the ROC curve for the LDA classifier on
...
Table [17](#orgffc9a22) shows the possible results when applying a
classifier (or diagnostic test) to a population.  

<table id="orgffc9a22" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
...
Table [18](#org25d42eb) lists many of the popular performance measures that
are used in this context.

<table id="org25d42eb" border="2" cellspacing="0" cellpadding="6" rules="groups" frame="hsides">
<caption class="t-above"><span class="table-number">Table 18:</span> Important measures for classification and diagnostic testing, derived from quantities in table [17](#orgffc9a22).</caption>
...
Figure [131](#orgd5acf50) illustrates the performances of LDA and QDA in two
...
Figure [133](#org90d7cdb) illustrates the performances of the four
...
Figure [142](#org7a0b29b) displays the *validation set approach*, a simple
...
In figure [144](#org879bb89), the left-hand panel shows validation sample MSE as a
...
Figure [146](#org4a242b1) displas the Leave One Out Cross Validation (LOOCV) approach.
...
The left-hand panel of figure [148](#org450e153) shows test set MSE as a function of
...
The right-hand panel of figure [148](#org450e153) shows nine different 10-fold
...
Figure [150](#orgcecf928) illustrates the *k*-fold CV approach.
