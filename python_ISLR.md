...
1.  [Introduction](#orgf9de9ee)
2.  [Statistical Learning](#org1cc281f)
    1.  [What is Statistical Learning?](#orgd843c4c)
    2.  [Assessing Model Accuracy](#orge336348)
...
Figure [2](#org46ebddc) shows graphs of Wage versus three variables. 
...
Figure [4](#orgdf26abf) shows boxplots of previous days' percentage changes in S&P
...
Figure [6](#org010fcd7) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [8](#org66fe557) is a plot of `Income` versus `Years of Education` from the
...
Figure [10](#org2555002) is a plot of `Income` versus `Years of Education` and
...
Figure [12](#orgab0f64d) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [10](#org2555002).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [14](#org78aa9a3) provides an illustration of the trade-off between
...
Figure [16](#orgd981003) provides a simple illustration of the clustering problem.
...
Figure [20](#org5a90bfe) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [20](#org5a90bfe), we see considerable
...
Figure [22](#orgaa44da1) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [20](#org5a90bfe) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [24](#orgc925c06) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [20](#org5a90bfe), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [26](#orgaaddd36) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [20](#org5a90bfe) - [24](#orgc925c06).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")
