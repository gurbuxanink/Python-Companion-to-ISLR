...
1.  [Introduction](#org3ca5211)
2.  [Statistical Learning](#org90a76ba)
    1.  [What is Statistical Learning?](#org5db6ea2)
    2.  [Assessing Model Accuracy](#orge961e3a)
    3.  [Lab: Introduction to Python](#org83c00d1)
        1.  [Basic Commands](#org7cfede6)
        2.  [Graphics](#org5cb2a62)
        3.  [Indexing Data](#org3979816)
        4.  [Loading Data](#orgfd0c5ba)
        5.  [Additional Graphical and Numerical Summaries](#org220088d)
...
Figure [2](#orgb68e9c0) shows graphs of Wage versus three variables. 
...
Figure [4](#org6bac13c) shows boxplots of previous days' percentage changes in S&P
...
Figure [6](#org0e10dfe) shows scatter plots of `sales` versus `TV`, `radio`,
...
Figure [8](#org3e3971f) is a plot of `Income` versus `Years of Education` from the
...
Figure [10](#org47958c5) is a plot of `Income` versus `Years of Education` and
...
Figure [12](#orged7b7fc) shows an example of the parametric approach applied to
the `Income` data from previous figure. 

![img](figures/fig2_4.png "A linear model fit by least squares to the `Income` data from figure [10](#org47958c5).  The observations are shown in red, and the blue plane indicates the least squares fit to the data.")

Figure [14](#org12be2b9) provides an illustration of the trade-off between
...
Figure [16](#org99bb1cc) provides a simple illustration of the clustering problem.
...
Figure [20](#orgfcceee6) illustrates the tradeoff between training MSE and test
...
When we repeat the simulations for figure [20](#orgfcceee6), we see considerable
...
Figure [22](#orge0c6443) provides another example in which the true \(f\) is
approximately linear. 

![img](figures/fig2_10.png "Details are as in figure [20](#orgfcceee6) using a different true \(f\) that is much closer to linear.  In this setting, linear regression provides a very good fit to the data.")

Figure [24](#org5063695) displays an example in which \(f\) is highly
non-linear. The training and test MSE curves still exhibit the same general
patterns.

![img](figures/fig2_11.png "Details are as in figure [20](#orgfcceee6), using a different \(f\) that is far from linear.  In this setting, linear regression provides a very poor fit to the data.")

Figure [26](#org3fd4611) displays the relationship between bias, variance, and
...
![img](figures/fig2_12.png "Squared bias (blue curve), variance (orange curve), \(Var(\epsilon)\) (dashed line), and test MSE (red curve) for the three data sets in figures [20](#orgfcceee6) - [24](#org5063695).  The vertical dotted line indicates the flexibility level corresponding to the smallest test MSE.")

Figure [28](#org4eea6ad) provides an example using a simulated data set in
...
Figure [30](#org82244ae) displays the KNN decision boundary, using \(K=10\), when
applied to the simulated data set from figure [28](#org4eea6ad).  Even though
...
![img](figures/fig2_15.png "The firm line indicates the KNN decision boundary on the data from figure [28](#org4eea6ad), using \(K = 10\). The Bayes decision boundary is shown as a dashed line.  The KNN and Bayes decision boundaries are very similar.")

![img](figures/fig2_16.png "A comparison of the KNN decision boundaries (solid curves) obtained using \(K=1\) and \(K=100\) on the data from figure [28](#org4eea6ad).  With \(K=1\), the decision boundary is overly flexible, while with \(K=100\) it is not sufficiently flexible.  The Bayes decision boundary is shown as dashed line.")

In figure [33](#org92109ec) we have plotted the KNN test and training errors as
...
![img](figures/fig2_17.png "The KNN training error rate (blue, 200 observations) and test error rate (orange, 5,000 observations) on the data from figure [28](#org4eea6ad) as the level of flexibility (assessed using \(\frac{1}{K}\)) increases, or equivalently as the number of neighbors \(K\) decreases.  The black dashed line indicates the Bayes error rate.")
